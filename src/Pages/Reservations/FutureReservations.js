...
function FutureReservations() {
    ...
    const fetchReservations = async () => {
      ...
      try {
        const {data} = await axios.get(
          `${process.env.REACT_APP_API}/reservation?page=1&limit=${limit}&sortBy=createdAt&sortOrder=1&time=future&userId=${state.user._id}`,
          {
            headers: {
  ...
  const handleClickPage = async ({selected}) => {
    ...
    try {
      const {data} = await axios.get(
        `${process.env.REACT_APP_API}/customer?page=${page}&limit=${limit}&sortBy=firstName&sortOrder=1&time=future&userId=${state.user._id}`,
        {
          headers: {
            Authorization: `Bearer ${state.token}`,
          },
